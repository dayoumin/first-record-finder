...
const LLM_PROVIDERS: { value: LLMProvider; label: string; models: string[]; needsKey: boolean }[] = [
  { value: 'ollama', label: 'Ollama (로컬)', models: ['llama4', 'llama3.3', 'qwen2.5', 'gemma3'], needsKey: false },
        ...
        {status === 'completed' && completed > 0 && (
          <section className="pdf-section">
            ...
            <div className="pdf-upload">
              ...
              <span className="upload-hint">여러 파일을 한번에 선택할 수 있습니다 (최대 50MB/파일)</span>
            </div>
            ...
            {uploadedPDFs.length > 0 && (
              <div className="pdf-list">
                <div className="pdf-list-header">
                  <span>업로드된 문헌 ({uploadedPDFs.length}개)</span>
                  <button
            ...
            {uploadedPDFs.length === 0 && (
              <div className="pdf-empty">
                <p>PDF 문헌을 업로드하여 한국 기록 여부를 분석하세요.</p>
                <p className="hint">
                  {selectedSpecies
                    ? `"${selectedSpecies}" 관련 문헌을 업로드하세요.`
                    : '먼저 분석할 학명을 선택하세요.'}
                </p>
              </div>
            )}
